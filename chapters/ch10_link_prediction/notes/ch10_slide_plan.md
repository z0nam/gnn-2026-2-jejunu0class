...
- [ ] 슬라이드 2 (표본 프레이밍) — 초안 작성됨, 분량/표현 다듬기
- [ ] 슬라이드 5 수정 + 5b (AUC/AP) — 채택 여부, 분량 결정
- [ ] 슬라이드 8 (내적 디코더 가정) — 채택 여부 결정
- [ ] 슬라이드 15 (4-cycle/1-WL) — 채택 여부 결정, 그림 필요할 수도 (tikz 4-cycle)
- [ ] 비교표 행 추가 문구 확정
- [ ] 전체 분량 점검: 현재 24장 + 신규 3장 = 27장. 발제 시간 대비 적정한지
- [ ] 확정 후 `ch10_link_prediction_beamer_frames.tex`에 일괄 반영 + 빌드 (`scripts/build.sh`)
