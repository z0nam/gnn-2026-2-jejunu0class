...
\begin{itemize}
  ...
  \item 벤치마크의 뒤틀림: 정답 그래프를 알면서 \texttt{RandomLinkSplit}으로 \emph{일부러 숨김} = \textbf{MCAR}(Missing Completely At Random) 가정. 현실 결측은 대개 \textbf{MNAR}(Missing Not At Random — 결측이 특정 영역에 몰림) $\to$ 벤치마크가 실전을 과대평가할 수 있음
  \item 음성 라벨도 사실 없음: 비관측 쌍 = 진짜 비엣지 \emph{or} 결측 엣지 — \textbf{PU-learning}(Positive-Unlabeled) 구조
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item \textbf{AUC}(Area Under the ROC Curve) $= P\bigl(s(\text{무작위 양성}) > s(\text{무작위 음성})\bigr)$ — 양성·음성 쌍 대결에서 양성이 이길 확률. $0.5$ = 무작위, $1.0$ = 완벽한 분리
  \item \textbf{AP}(Average Precision) = 점수 내림차순으로 읽어내려가며 양성을 만날 때마다 그 시점의 precision을 평균 — \emph{순위 상위권 오염에 민감}
\end{itemize}
\vspace{0.3em}
...
\begin{frame}{DGCNN(Deep Graph CNN): SEAL의 분류기}
